...
function setConfiguration(apiName)
{
    ...
    webinos.configuration.setServiceConfiguration(apiName, JSON.parse(config).params, successCB, errorCB);
...
//            div.appendChild(convert(form, data.template, data_));
        ...
        for (var d in data)
        {
            ...
            webinos.configuration.getServiceConfiguration(data[d], fillConfigurationData, errorCB);
        }
    };

    webinos.discovery.findConfigurableAPIs("*", fillHeader, errorCB);
} 
